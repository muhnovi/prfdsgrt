...
import { X, Youtube, ChevronUp, ChevronDown, Share2, Check, MapPin } from "lucide-react"
...
export default function NewsDetailModal({ news, isOpen, onClose }: NewsDetailModalProps) {
  ...
  const handleViewOnMaps = () => {
    if (news?.mapsUrl) {
      window.open(news.mapsUrl, "_blank", "noopener,noreferrer")
    }
  }
      ...
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-in zoom-in-95">
        ...
        <div className="sticky top-0 bg-white border-b border-border p-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={handleShare}
              className={`p-2 rounded-lg transition-colors ${isShared ? "bg-green-100" : "hover:bg-muted"}`}
            ...
            <button onClick={onClose} className="p-2 hover:bg-muted rounded-lg transition-colors" aria-label="Tutup">
        ...
        <div className="p-6">
          <div className="mb-6 overflow-hidden rounded-lg bg-muted">
            <img
              src={news.image || "/placeholder.svg?height=400&width=600&query=berita desa detail"}
              alt={news.title}
          ...
          {news.youtubeUrl && (
            <div className="mb-6 p-4 bg-muted rounded-lg">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-semibold text-foreground">Video Terkait</span>
              </div>
              <a
